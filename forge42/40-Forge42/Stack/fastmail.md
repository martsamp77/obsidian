...
## dimension42.ai Setup

### DNS Configuration

Point `dimension42.ai` MX records to FastMail servers. In FastMail: **Settings → My Email Addresses → Add Email Domain**.
...
Also add the FastMail SPF and DKIM records for deliverability.

### Catch-All Wildcard

Enable `*@dimension42.ai` catch-all after adding the domain. Route all catch-all to a dedicated **D42** folder in FastMail.

Folder rule: Recipient address contains `@dimension42.ai` → Move to **D42**

### Purpose-Built Sending Identities

Create these as sending identities in **Settings → My Email Addresses**:

| Address | Purpose |
|---|---|
| `hello@dimension42.ai` | General contact, about page |
| `support@dimension42.ai` | Product support |
| `eddie@dimension42.ai` | Eddie AI agent persona outbound |
| `no-reply@dimension42.ai` | Automated system notifications |
| `[agentname]@dimension42.ai` | Any future BAAS agent persona |

### What NOT to Do

- **Do not move 37metrics.com email to FastMail** — it stays on Office 365 (legal entity email, works fine, gives you Office)
- **Do not add martysampson.com to FastMail** unless there's a specific need — Gmail SSO is fine for the personal brand